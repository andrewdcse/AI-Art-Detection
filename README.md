...
The final method we used was transfer learning from a pre-trained ResNet model. The imported model was trained on the data for varied epochs and achieved high performance.
...
I initially had low expectations for model performance on this dataset, even for transfer learning. I reasoned that ImageNet training would carry over to recognizing the small nuances between real and generated images. However, it is clear that transfer learning is very effective at this task, reaching over 90% accuracy. The first problems I encountered were with setting up the dataset and models in Google Colab, since it was the first time I had done this and I ran into various small issues relating to loading and storing the dataset, splitting the data into training and test sets, and transforming those sets separately. If I were to continue with this project, I would likely try to find or gather a larger dataset of images generated by various models. I might also try making a user interface that allows for images to be uploaded and evaluated by the model. The main difference that my approach had from others is that I tried weaker models before more powerful and complex ones, and this allowed me to see the differences with between these methods. Exploring the convlutional network parameters also helped me better understand the effects of those parameters.
